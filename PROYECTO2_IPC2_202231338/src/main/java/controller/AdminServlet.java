...
public class AdminServlet extends HttpServlet {
    ...
    @Override
        ...
        try {
            ...
            (user.getRoleId() == 3)) {
                System.out.println("login correcto Admin");
                System.out.println(username+" , "+password);
            // Usuario autenticado correctamente
            request.getSession().setAttribute("user", user);
            response.sendRedirect(request.getContextPath() + "/Administracion/personal.jsp");


        } else {
                System.out.println("login incorrecto Venta");
            // Redirigir con mensaje de error
            response.sendRedirect("/Administracion/personal.jsp?error=1");
        }
